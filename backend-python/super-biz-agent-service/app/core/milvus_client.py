...
class MilvusClientManager:
    ...
    def connect(self) -> MilvusClient:
        ...
        try:
            ...
            if not self._collection_exists():
                ...
                if not self._is_schema_compatible(self._collection.schema):
                    logger.warning(f"检测到 collection '{self.COLLECTION_NAME}' schema 与当前 RAG 结构不兼容，准备重建")
    ...
    def _create_collection(self) -> None:
        ...
        fields = [
            ...
            FieldSchema(
                name="chunk_id",
                dtype=DataType.VARCHAR,
                max_length=self.CHUNK_ID_MAX_LENGTH,
            ),
            FieldSchema(
                name="document_id",
                dtype=DataType.VARCHAR,
                max_length=self.DOC_ID_MAX_LENGTH,
            ),
            FieldSchema(
                name="knowledge_base_id",
                dtype=DataType.VARCHAR,
                max_length=self.KB_ID_MAX_LENGTH,
            ),
            FieldSchema(
                name="content",
                dtype=DataType.VARCHAR,
            ...
            FieldSchema(
                name="section_path",
                dtype=DataType.VARCHAR,
                max_length=self.SECTION_PATH_MAX_LENGTH,
            ),
            FieldSchema(
                name="file_name",
                dtype=DataType.VARCHAR,
                max_length=self.FILE_NAME_MAX_LENGTH,
            ),
            FieldSchema(
                name="metadata",
                dtype=DataType.JSON,
    ...
    def _is_schema_compatible(self, schema: CollectionSchema) -> bool:
        expected_fields = {
            "id": DataType.VARCHAR,
        ...
        if set(expected_fields) - set(fields):
            return False
        for field_name, dtype in expected_fields.items():
            if fields[field_name].dtype != dtype:
                return False
        vector_field = fields["vector"]
        existing_dim = getattr(vector_field, "params", {}).get("dim")
        return existing_dim == self.VECTOR_DIM
